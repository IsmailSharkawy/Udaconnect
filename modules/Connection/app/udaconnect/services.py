...
class ConnectionService:
    ...
    def find_contacts(person_id: int, start_date: datetime, end_date: datetime, meters=5
        ...
        logger.warning('person_map',person_map)
...
class PersonService:
    ...
    def retrieve_all() -> List[Person]:
        ...
        logger.warning('stub',stub)
        response = stub.Retrieve(person_pb2.Empty())
        logger.warning('response',response)
        grpc_res = []
        # logger.warning('response',list(response)) 
        for person in response:
            logger.warning('grpc_append',grpc_res) 
            grpc_res.append(person)
            
        logger.warning('grpc_res',grpc_res)     
        return grpc_res 
